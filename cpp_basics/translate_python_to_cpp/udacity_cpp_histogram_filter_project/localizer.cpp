	...
	float p_miss) 
{
  ...
  for (int r = 0; r < h; ++r){
    for (int c = 0; c < w; ++c){
      if (grid[r][c]==color){
          newGrid[r][c]=beliefs[r][c]*p_hit;
      }else{
          newGrid[r][c]=beliefs[r][c]*p_miss;
      }
    }
  }
	// your code here
	...
	float blurring) 
{
  int h = beliefs.size();
  int w = beliefs[0].size();
	vector < vector <float> > newGrid(w, vector<float>(h, 0.0));

  for (int r = 0; r < h; ++r){
    for (int c = 0; c < w; ++c){
      float cell = beliefs[r][c];
      int new_r = (r + dx) % h;
      int new_c = (c + dy) % w;
      newGrid[new_c][new_r] = cell;
    }
  }
	// your code here
